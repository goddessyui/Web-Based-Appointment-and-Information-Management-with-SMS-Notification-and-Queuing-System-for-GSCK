            ...
            if($countapp<=($limit-1)){                         
               ...
               if (mysqli_query($db, $updatedate)) {
                  ...
                  if (mysqli_query($db, $insertnew)) {
                     ...
                     header('location: ../unifast_missed_request.php?error="<?php echo "Error inserting record " . mysqli_error($db);?>"');
                  }
               } 

               else {
                 
                  header('location: ../unifast_missed_request.php?error="<?php echo "Error updating record " . mysqli_error($db);?>"');
                
               }
            }
            else{
               header('location: ../unifast_missed_request.php?error="Appointment for that date are already limited!"');
            }
         }  
      }
   }
