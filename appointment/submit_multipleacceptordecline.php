         ...
         while($al=mysqli_fetch_assoc($limitvalue)) {   
            ...
            if(isset($_POST['accept'])) {//accept appointment
               ...
               if (empty($_POST['pending'])) {
                  header('location: ../unifast_submit_pending.php?error="Please check at least one appointment."'); 
               }
               else{
                     ...
                     foreach ($_POST['ad'] as $ad) {
                        ...
                        foreach ($_POST['com'] as $com) {
                           ...
                           if (empty($appointment_date)) {//if appointment date is not filled
                              header('location: ../unifast_submit_pending.php?error="Appointment date should be filled."'); 
                           }
                           else {
                              ...
                              if($countapp<=($limit-1)) {
                                 ...
                                 if(mysqli_query($db, $acceptappointment)) {
                                    ...
                                    $querys = mysqli_query($db, "SELECT tbl_staff_registry.first_name, tbl_staff_registry.last_name FROM tbl_staff_registry WHERE staff_id='".$staff_id."'");
                                       ...
                                       $accept='';
                                       header('location: ../unifast_submit_pending.php?success="Appointment request accepted."'); 
                                 } 
                                 else {
                                    header('location: ../unifast_submit_pending.php?error="<?php echo "ERROR: Not able to execute. " . mysqli_error($db);?>"');
                                 }
                              }
                              else {
                                    header('location: ../unifast_submit_pending.php?error="Appointments are limited for this date."');
                              }
                        }
                           ...
                           $requests="SELECT tbl_appointment.appointment_id, tbl_appointment.date_created,
                              ...
                              if(mysqli_query($db, $declineappointment)) {
                                 ...
                                 $querys = mysqli_query($db, "SELECT tbl_staff_registry.first_name, tbl_staff_registry.last_name FROM tbl_staff_registry WHERE staff_id='".$staff_id."'");
                                    ...
                                    include ('sms_unifast.php');
                                    $decline='';
                                    header('location: ../unifast_submit_pending.php?success="Appointment request declined."');    
                              } 
                              else { 
                                 header('location: ../unifast_submit_pending.php?error="<?php echo "ERROR: Not able to execute. " . mysqli_error($db);?>"');
                              }
