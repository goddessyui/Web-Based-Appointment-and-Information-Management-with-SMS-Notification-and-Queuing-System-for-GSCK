...
if (isset($accept)){
    ...
    $message = "accepted your request for ".$appointment_type." on " 
    . date("F d, Y", strtotime($appointment_date)).' from '. date("h:ia", strtotime($appointment_time_open)).' - '. date("h:ia", strtotime($appointment_time_close)).
    " only. ".$queuenumber;
    ...
    $message =  "declined your request for ".$appointment_type;
    $send = new smsfunction();
    $send->itexmo($receiver, $message, $smsAPICode, $smsAPIPassword);
}
    ...
    $message = "rescheduled  for ".$appointment_type.". on " . date("F d, Y", strtotime($newDate)).' from '. date("h:ia", strtotime($appointment_time_open)).' - '. date("h:ia", strtotime($appointment_time_close))." only.".$queuenumber;
    $send = new smsfunction();
    $send->itexmo($receiver, $message, $smsAPICode, $smsAPIPassword);
}
    ...
    $message = "has cancelled the appointment for " .$appointment_type;
    $send = new smsfunction();
    $send->itexmo($receiver, $message, $smsAPICode, $smsAPIPassword);
}
